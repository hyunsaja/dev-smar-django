...
class CodeAdmin(admin.ModelAdmin):
    ...
    def show_firm_url(self, obj):
        return format_html(
            "<a href='%s/reply/'>%s</a>" % (obj.aNumber, "Reply"))
    ...
    def get_urls(self):
        ...
        post_url = [
            path('<path:object_id>/reply/',
                 self.admin_site.admin_view(self.reply_view),
                 name='%s_%s_reply' % info),  # SHS 댓글용 추가
    ...
    # create 댓글용
    def reply_view(self, request, object_id, form_url='',
                   extra_context=None):  # SHS 댓글용
    ...
    def my_view(self, request):
        ...
        print(context)
        return TemplateResponse(request, "admin/code/myView.html",
                                {'ctx': context})
    ...
    def save_model(self, request, obj, form, change):
        ...
        if self.is_reply:
            ...
            Code.objects.filter(gNumber=refObj.gNumber,
                                gSubNumber__gt=refObj.gSubNumber).update(
                gSubNumber=F('gSubNumber') + 1)
